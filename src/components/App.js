...
// Import songs
import song1 from "../static/songs/Naatu Naatu.mp3"
import song2 from "../static/songs/Bones.mp3"


// Import song cover images
import song1Img from "../static/Naatu Naatu.jpg";
import song2Img from "../static/Bones.jpg";


// Import wallpapers
import HomeScreen from "../static/HomeScreen.jpg"
...
class App extends React.Component {
  constructor() {
    ...
    this.state = {
      ...
      wallpaperItems: [HomeScreen], //wallpapers
      songItems: ["Naatu Naatu","Bones"], //song names
      songIndex: 0, //current song
      lengthMenuKey: { "-1": 3, 1: 2, 4: 4, 3: 2},  //length of a particular menu
      menuMapping: { "-1": [0, 1, 2, 3], 1: [4, 5, 6]}, //which menu can be rendered by key menu
      ...
      theme: "rgb(49,90,111)", //current body theme
  ...
  render() {
    const { audio, active, currentMenu, menuItems, musicItems, songItems, playing, songIndex, theme, songUrl, songImgUrl, wallpaper, wallpaperItems} = this.state;
    return (
      <div className="App">
        <Case songIndex={songIndex} active={active} menuItems={menuItems} musicItems={musicItems} currentMenu={currentMenu} changeMenuForward={this.changeMenuForward} changeMenuBackward={this.changeMenuBackward} updateActiveMenu={this.updateActiveMenu} togglePlayPause={this.togglePlayPause} songItems={songItems} playing={playing} theme={theme} audio={audio} songUrl={songUrl} songImgUrl={songImgUrl} seekSongForward={this.seekSongForward} seekSongReverse={this.seekSongReverse} wallpaper={wallpaper} wallpaperItems={wallpaperItems} />
      </div>
    );
  }
}
